...
$(function () {
    ...
    $(window).on('load scroll resize', function () {
        ...
        if (window.matchMedia('(max-width: 991px)').matches) {
            $('.services-list, .why-list, .popular .car-list').addClass('owl-carousel').owlCarousel({
                items: 1,
                margin: 30,
                nav: true,
                autoHeight: true
            ...
            $('.services-list, .why-list, .popular .car-list').removeClass('owl-carousel').owlCarousel('destroy');
        }
    });
    ...
    $('.fleet-nav-item').on('click touchstart', function () {
        $(this).addClass('active').siblings().removeClass('active');
        var target = $(this).attr('data-target');
        if (target == 'all') {
            $('.fleet .car-list > div').show();
        } else {
            $('.car-card[data-id=' + target + ']').parent().show();
            $('.car-card:not([data-id=' + target + '])').parent().hide();
        }
    });

    $('.transfers-slider').owlCarousel({
        nav: true,
        dots: false,
        loop: true,
        margin: 30,
    ...
    $('.locations-block:first-child, .accordion-item:first-child .locations-list-item:first-child').addClass('active');

    $('.accordion-item-toggle').on('click', function () {
        $(this).siblings('.accordion-item-cont').slideToggle('fast');
        $(this).closest('.accordion-item').toggleClass('active').siblings().removeClass('active').find('.accordion-item-cont').slideUp('fast');
    });

    $('.locations-list-item').on('click', function () {
        $(this).addClass('active').siblings().removeClass('active').closest('.accordion-item').siblings().find('.locations-list-item').removeClass('active');
        var target = $(this).attr('data-target');
        $('.locations-block[data-id=' + target + ']').addClass('active').siblings().removeClass('active');
        if (window.matchMedia('(max-width: 1359px)').matches) {
            $('html,body').animate({scrollTop: $('.locations-right').offset().top - 120}, 'slow');
        }
    });
